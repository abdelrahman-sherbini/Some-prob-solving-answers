...
vector<string> summ,endsum;
for(auto j:stt){
lonn(stt,j,j,summ);
for(auto i :summ){
    if (i.size()==n){
    // summ[i].erase(summ[i].begin()+n);
    // cout<<i;
    endsum.pb(i);}
}
summ.clear();
...
bool flagq = false;
string mem2;
vector<string> finresult,stt2=stt;
for(auto i :endsum){
    stt2=stt;
    for(int k = 0; k < n-2; k++)
    {
        mem2 = i.substr(k,3);
        if((find(stt2.begin(), stt2.end(), mem2) != stt2.end())) {
            auto itr = std::find(stt2.begin(), stt2.end(), mem2);
if (itr != stt2.end()) stt2.erase(itr);
            continue;}
        flagq = true;
        break;
    }
    if (!flagq&stt2.empty()){
        finresult.pb(i.substr(0,i.size()));
    }
    flagq = false;
}
if(finresult.size()==0){
