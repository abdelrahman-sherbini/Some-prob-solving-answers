...
string nn,iop;
getline(cin,nn);
vector<string> iopstt,stt=split(nn,' ',true);
vector<int> input;
for(auto i :stt){
input.pb(stoi(i));
}
vector<vector<int>> points(input[0]);
flush
for(int i = 0; i < input[0]; i++)
{
    getline(cin,iop);
    iopstt=split(iop,' ',true);
    points[i] = vector<int>(2,0);
    for(int j = 0; j < 2; j++)
    {
        points[i][j] = stoi(iopstt[j]);
    }
    iop.clear();
}

vector<vector<int>> finot;
vector<int> phs;
...
for(int i = 0; i < input[0]; i++)
{
    ...
    {
      phs.clear();
    if(abs(points[i][0]-points[i+j][0])<=input[1]){
        // cout<< " if"<<i<<" ";
phs.pb(points[i][0]);
phs.pb(points[i][1]);
phs.pb(points[i+j][0]);
...
for(auto i :finot){
// cout<<i<<" ";
ma = i[1] + i[3]+abs(i[2]-i[0]);
maxx.pb(ma);
}
if(!maxx.empty())
cout<<*max_element(maxx.begin(), maxx.end()-1);
else cout<<-1;
}